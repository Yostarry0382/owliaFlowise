...
export const nodeReferences: Record<string, NodeReference> = {
  ...
  humanReview: {
    description:
      'AIの出力を人間が確認・承認するためのノードです。重要な決定の前に人間の判断を挟むことができます。',
    features: [
      '人間による確認・承認',
      '出力の編集機能',
      'タイムアウト設定',
    ...
    tips: [
      '承認メッセージを明確にして確認者が判断しやすくする',
      'タイムアウトを設定して処理が止まることを防ぐ',
    ],
  },
  ...
  agentAsTool: {
    description:
      'OwlAgentをLLMエージェントのツールとして使用可能にするノードです。他のエージェント（ConversationalAgentなど）がこのツールを呼び出してOwlAgentを実行できます。',
    features: [
      'OwlAgentをツールとしてラップ',
      'ツール名と説明のカスタマイズ',
      'Return Direct機能（結果を直接返す）',
      'OpenAI Function Calling互換のスキーマ生成',
      'ネストされたエージェント実行',
    ],
    useCases: [
      '専門的なタスクをツール化',
      ...
      'エージェントの階層化',
      '複雑なワークフローの分散処理',
      '特定のドメインに特化したサブエージェント',
    ],
    tips: [
      'ツールの説明を明確にすると、LLMが適切なタイミングでツールを選択します',
      'Return Directを有効にすると、ツールの結果が最終出力として直接返されます',
      'ツール名は英数字とアンダースコアのみを使用してください',
      '複雑なタスクは複数のツールに分割すると効率的です',
    ],
  },
};
